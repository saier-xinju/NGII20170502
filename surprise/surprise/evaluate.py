...
class GridSearch:
    ...
    def __init__(self, algo_class, param_grid, measures=['rmse', 'mae'],
        ...
        if 'sim_options' in param_grid:
            ...
            sim_options_list = [dict(list(zip(sim_options, v))) for v in
                                product(*list(sim_options.values()))]
            param_grid['sim_options'] = sim_options_list

        if 'bsl_options' in param_grid:
            bsl_options = param_grid['bsl_options']
            bsl_options_list = [dict(list(zip(bsl_options, v))) for v in
                                product(*list(bsl_options.values()))]
            param_grid['bsl_options'] = bsl_options_list

        self.param_combinations = [dict(list(zip(param_grid, v))) for v in
                                   product(*list(param_grid.values()))]
    ...
    def evaluate(self, data):
        ...
        for param, score in zip(params, scores):
            for param_key, score_key in zip(list(param.keys()), list(score.keys())):
